...
from .graal_study_evolution import (get_unique_repository,
                                    get_to_date,
                                    get_cumulative_issues_statut)
...
#        return "id"
    ...
    def enrich_backlog_analysis(self, ocean_backend, enrich_backend, no_incremental=False,
                              out_index="github_enrich_backlog", interval_months=[1],
        ...
        for repository_url in repositories:
            ...
            cumulative_issues_statut = es_in.search(
                index=in_index,
                body=get_cumulative_issues_statut(repository_url)
            )['aggregations']['created_per_month'].get("buckets", [])
            

            repository_name = repository_url.split("/")[-1]
            
            evolution_items = []
            for statut in cumulative_issues_statut:
              date = statut['key_as_string']
              evolution_item = {
                "uuid": "{}_{}".format(date, repository_name),
                'cumulative_created': statut['cumulative_created']['value'],
                'cumulative_closed': statut['cumulative_closed']['value'],
                "cumulative_opened": statut['cumulative_created']['value']-statut['cumulative_closed']['value'],
                'created': statut['created']['value'],
                'closed': statut['closed']['value'],
                "opened": statut['created']['value']-statut['closed']['value'],
            ...
            last_date = str_to_datetime(evolution_item['study_creation_date']).replace(tzinfo=None) 
